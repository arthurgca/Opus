...
// D3 Component Setup
let g = svg.append("g"),
  link = g.append("g").attr("stroke", "#000000").style("stroke-width", 3).selectAll(".link"),
...
function update() {
  ...
  // Matching subgenres make the link thicker after first node
  link.style("stroke-width", function (d) { return returnMatches(d).length > 0  ? returnMatches(d).length * 3 : 3  });

  // Show matching genres between links on a tooltip
  link.on("mouseover",showTooltip)
      .on("mousemove",moveTooltip)
      .on("mouseout",hideTooltip);

  image.on("mouseover",showTooltipNode)
      .on("mousemove",moveTooltip)
      .on("mouseout",hideTooltip);
...
function setSelected(root) {
  ...
  let rootLinks = link.filter(function(d) {
    ...
    rootNode.style("fill", "#19FCFF").attr("r", function(d) { return d.popularity });
    //rootLinks.style("stroke-width", function (d) { return returnMatches(d).length > 0  ? returnMatches(d).length * 3 : 3  });
}
...
function showTooltip(d) {
  ...
  tooltip.style("display","block")
      .text("Matching genres: "+returnMatches(d));
}

function showTooltipNode(d) {
  moveTooltip();
  tooltip.style("display","block");
  tooltip.html("<img src="+d.images[1].url+"></img>");
}
...
// return the matching genres between a link and his targets
function returnMatches(d) {
    if (Object.keys(clickedIds).length <= 1 || d.source.genres === undefined) {
      return NODES_OBJ[d.source].genres.filter(element => NODES_OBJ[d.target].genres.includes(element));
    }
    else if(d.source.genres != undefined) {
      return d.source.genres.filter(element => d.target.genres.includes(element));
      }
    return 0;
}
